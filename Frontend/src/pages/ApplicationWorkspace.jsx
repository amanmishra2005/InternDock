...
import { ArrowLeft, CheckCircle2, FileText, Send, Check, CreditCard, ShieldCheck, QrCode, X, Copy, BookOpen, ChevronDown, ChevronUp, Sparkles, Clock, ExternalLink } from "lucide-react";
import api from "../api/axios";
import OfferLetterPreview from "../components/OfferLetterPreview";
import CertificatePreview from "../components/CertificatePreview";

const PROGRESS_STEPS = ["Submitted", "Under Review", "Selected", "Active", "Completed"];
...
function AssignmentCardItem({ assignment, submission, onSubmit }) {
  ...
  return (
    <motion.div whileHover={{ y: -3 }} className="glass-card assignment-card-item">
      <div className="assignment-header flex items-center justify-between">
        <span className="badge-pill badge-pill-cyan">{assignment.weekLabel ? `${assignment.weekLabel} Milestone` : `Milestone ${assignment.week}`}</span>
        {submission ? (
      ...
      >
        ...
        <span>{showInstructions ? "Hide Task Guidance" : "View Step-by-Step Task Guidance"}</span>
            ...
            <div className="instructions-steps-list">
              {instructionLines.map((step, idx) => (
                <div key={idx} className="step-instruction-item">
                  ...
                  <span className="step-text-val">{step.replace(/^\d+\.\s*/, "")}</span>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <form onSubmit={handleSub} className="assignment-form">
        <div className="input-group" style={{ marginBottom: "0.5rem" }}>
          <label className="input-label flex items-center justify-between">
            ...
            {submission?.githubUrl && (
              <a href={submission.githubUrl} target="_blank" rel="noreferrer" className="link-external-view">
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="btn btn-primary btn-sm full-width btn-glow">
          <span>{submission ? "Update Solution" : "Submit GitHub Solution"}</span>
          <Send size={14} />
        </motion.button>
      </form>
...
export default function ApplicationWorkspace() {
  ...
  const load = () => {
    Promise.all([
      ...
      api.get(`/assignments/for-application/${id}`),
    ])
      .then(([appRes, assRes]) => {
        setApplication(appRes.data);
        setAssignments(assRes.data);
        if (appRes.data.student) {
          setPaymentForm({
            payerName: appRes.data.student.fullName || "",
            registeredEmail: appRes.data.student.email || "",
            utrNumber: "",
          });
        }
      })
  ...
  const submitAssignment = async (assignmentId, payload) => {
    try {
      await api.post(`/assignments/${assignmentId}/submit`, { applicationId: id, ...payload });
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      setMessage("✓ Payment confirmed via UPI! Program status updated to Active.");
  ...
  const submitFinalReport = async (e) => {
    e.preventDefault();
    try {
      await api.post("/documents/final-report", { applicationId: id, ...reportForm });
  ...
  const downloadOfferLetter = async () => {
    try {
      ...
      const downloadUrl = pdfPath.startsWith("http") ? pdfPath : pdfPath.startsWith("/") ? pdfPath : `/${pdfPath}`;
      window.open(downloadUrl, "_blank");
    } catch (err) {
      console.error("Error downloading offer letter:", err);
      alert(err.response?.data?.message || "Could not generate offer letter PDF.");
    }
  };
  ...
  const downloadCertificate = async () => {
    try {
      ...
      alert(err.response?.data?.message || "Could not generate certificate PDF.");
    }
  };
  ...
  return (
    <div className="container workspace-container">
      ...
      >
        <div>
          <span className="badge-pill badge-pill-cyan">{application.domain?.category}</span>
          <h1 className="workspace-title">{application.domain?.name} Workspace</h1>
          <p className="workspace-subtitle">
            Application ID: <strong className="id-highlight">{application.applicationId}</strong> • {application.duration?.label} Track
          </p>
        </div>
      ...
      >
        ...
        <div className="workspace-stepper">
          {PROGRESS_STEPS.map((s, i) => (
            <div key={s} className={`stepper-step ${i <= currentStepIndex ? "done" : ""}`}>
              <div className="step-circle">{i <= currentStepIndex ? <Check size={16} /> : i + 1}</div>
              <span className="step-text">{s}</span>
              {i < PROGRESS_STEPS.length - 1 && <div className="step-line"></div>}
            </div>
          ))}
        </div>
      </motion.div>
      ...
      {message && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="notice-banner">
          {message}
        </motion.div>
      )}
      ...
      {application.status === "Selected" && (
        ...
        >
          <div className="unlocked-text">
            <span className="badge-pill badge-pill-emerald">Offer Unlocked 🎉</span>
            <h2>Congratulations! You've been selected for {application.domain?.name}.</h2>
            <p>Your official InternDock selection offer letter is generated and cryptographically signed.</p>
          </div>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="btn btn-primary btn-glow" onClick={downloadOfferLetter}>
            <FileText size={18} />
      ...
      {["Active", "Completed", "Selected"].includes(application.status) && (
        <div className="workspace-main-content">
          <h2 className="section-header-title">Guided Internship Milestones ({assignments.length} Tasks + Capstone Final Report)</h2>
          {assignments.length === 0 ? (
            <div className="glass-panel text-center" style={{ padding: "2rem" }}>
              <p className="color-muted">No weekly assignments found for this domain track.</p>
            </div>
          ) : (
              ...
              {assignments.map(({ assignment, submission }, idx) => (
                ...
                >
                  <AssignmentCardItem
                    ...
                    onSubmit={(payload) => submitAssignment(assignment._id, payload)}
                  />
                </motion.div>
              ))}
            </div>
          ...
          >
            ...
            <p className="color-muted" style={{ marginBottom: "1.5rem" }}>
              Submit your final project write-up and source code URL to qualify for certificate issuance.
            </p>

            {application.finalReportSubmitted ? (
              <div className="report-submitted-badge">
                <CheckCircle2 size={18} />
                <span>Capstone Final Report Submitted &amp; Pending Review.</span>
              </div>
            ) : (
              <form onSubmit={submitFinalReport} className="report-form">
                ...
                <div className="input-group">
                  <label className="input-label">Executive Summary &amp; Introduction</label>
                  <textarea
                ...
                <div className="input-group">
                  <label className="input-label">Technical Work &amp; Features Completed</label>
                  <textarea
                    className="textarea-field"
                    rows="4"
                ...
                <div className="input-group">
                  <label className="input-label">GitHub Repository / Live Demo URL</label>
                  <input
                    className="input-field"
                    placeholder="https://github.com/username/repository"
                    value={reportForm.githubUrl}
                    onChange={(e) => setReportForm({ ...reportForm, githubUrl: e.target.value })}
                    required
                  />
                </div>

                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="btn btn-primary btn-glow full-width btn-lg mt-3">
          ...
          <div className="glass-panel payment-panel" style={{ marginTop: "2rem" }}>
            <h2>Program Fee &amp; Verification Status</h2>
            {application.paymentStatus === "Successful" ? (
              <div className="payment-success-pill">
                <CheckCircle2 size={18} />
                <span>Program Fee Paid (₹{application.duration?.fee}) • Account Verified</span>
              </div>
            ) : (
              <div className="payment-action-box">
                <p>Complete your program fee via UPI QR Code to unlock instant certificate eligibility upon capstone approval.</p>
                <motion.button 
                ...
                >
                  <QrCode size={18} />
                  <span>Pay Program Fee (₹{application.duration?.fee}) via UPI QR</span>
                </motion.button>
              </div>
            )}
          </div>
      ...
      {["Selected", "Active", "Completed"].includes(application.status) && (
        ...
        >
          <OfferLetterPreview
            ...
            startDate={application.startDate ? new Date(application.startDate).toDateString() : ""}
            endDate={application.endDate ? new Date(application.endDate).toDateString() : ""}
            applicationId={application._id}
            referenceId={application.offerLetterRef || `OFFER-${application._id?.slice(-6)}`}
          ...
          <div className="modal-backdrop">
            ...
            >
              <div className="modal-header">
                <div>
                  <span className="badge-pill badge-pill-emerald">Official Payment Gateway</span>
                  <h3 className="modal-title">Scan UPI QR &amp; Confirm Payment</h3>
                </div>
              ...
              <div className="modal-qr-grid">
                ...
                <div className="qr-image-card text-center">
                  <div className="qr-frame">
                    <img src="/payment_qr.png" alt="Aman Mishra UPI QR Code" className="payment-qr-img" />
                  </div>
                  <div className="upi-account-info">
                    <span className="account-holder-name">Aman Mishra</span>
                    <div className="upi-id-row">
                      <span className="upi-id-text">amanmishra15.08.2005-2@oksbi</span>
                      <button className="btn-copy-upi" onClick={copyUpiId} title="Copy UPI ID">
                        {copiedUpi ? <Check size={14} color="#10b981" /> : <Copy size={14} />}
                      </button>
                    </div>
                    <span className="pay-amount-tag">Amount to Pay: <strong>₹{application.duration?.fee}</strong></span>
                  </div>
                </div>

                {/* Form Container */}
                <form onSubmit={handlePaymentSubmit} className="payment-confirm-form">
                  <div className="input-group">
                  ...
                  <div className="input-group">
                    <label className="input-label">Registered Student Email</label>
                    <input
                      className="input-field"
                      type="email"
                      placeholder="student@example.com"
                      value={paymentForm.registeredEmail}
                      onChange={(e) => setPaymentForm({ ...paymentForm, registeredEmail: e.target.value })}
                      required
                    />
                  </div>

                  <div className="input-group">
                    <label className="input-label">UPI Reference / UTR Number (12 Digits)</label>
                    <input
                      className="input-field"
                  ...
                  <div className="payment-notice-box">
                    ...
                    <span>Scan QR code with any UPI App (GPay, PhonePe, Paytm, BHIM) and paste your UTR number above.</span>
                  </div>
                  ...
                  >
                    <span>{paymentSubmitting ? "Verifying Payment..." : `Confirm & Verify Payment (₹${application.duration?.fee})`}</span>
      ...
      <style>{`
        ...
        .banner-status-box { display: flex; flex-direction: column; align-items: flex-end; background: rgba(79, 70, 229, 0.08); border: 1px solid rgba(79, 70, 229, 0.2); padding: 0.75rem 1.25rem; border-radius: var(--radius-md); }
