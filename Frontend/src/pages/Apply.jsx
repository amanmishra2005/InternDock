...
import { ArrowLeft, ArrowRight, ShieldCheck, FileCheck, CheckCircle2, Clock } from "lucide-react";
...
export default function Apply() {
  ...
  useEffect(() => {
    api.get(`/domains/${slug}`).then((res) => {
      ...
      if (res.data.availableDurations?.length) {
        ...
        const matched = res.data.availableDurations.find((d) => d._id === queryDurationId);
        setDurationId(matched ? matched._id : res.data.availableDurations[0]._id);
      }
    });
  ...
  const submit = async (e) => {
    ...
    setError("");
    try {
      await api.post("/applications", { domainId: domain._id, durationId });
      navigate("/dashboard");
    } catch (err) {
  ...
  const selectedDuration = domain.availableDurations?.find((d) => d._id === durationId) || domain.availableDurations?.[0];
    ...
    <div className="container apply-page-container">
      ...
      >
        ...
        <p className="apply-subtitle">
          Your selected program track duration from domain details is locked below. Submit to generate your official InternDock selection offer letter &amp; workspace access.
        </p>
        ...
        <form onSubmit={submit} className="apply-form">
          ...
          <div className="selected-duration-badge-card">
            <div className="duration-card-header">
              ...
              <Link to={`/domains/${domain.slug}`} className="change-duration-link">Change Track</Link>
            </div>
            <div className="duration-card-body">
              <div>
                <h3 className="dur-name-label">{selectedDuration?.label || `${selectedDuration?.weeks} Weeks`} Track</h3>
                <p className="dur-desc-sub">Includes weekly capstone evaluation &amp; mentor feedback</p>
              </div>
          ...
          {selectedDuration && (
            ...
            >
              ...
              <div className="summary-row">
                ...
                <strong>{selectedDuration.label || `${selectedDuration.weeks} Weeks`} Track</strong>
              </div>
              <div className="summary-row">
          ...
          <motion.button 
            ...
            disabled={submitting}
          >
            <span>{submitting ? "Processing Application..." : `Confirm & Generate Selection Letter (₹${selectedDuration?.fee || 0})`}</span>
            <ArrowRight size={18} />
          </motion.button>
        </form>
      </motion.div>
